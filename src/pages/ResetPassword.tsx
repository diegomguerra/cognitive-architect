...
const ResetPassword = () => {
  ...
  useEffect(() => {
    ...
    import('@capacitor/app').then(({ App }) => {
      ...
      }).then((listener) => { capacitorListener = listener; });
    ...
    const timeout = setTimeout(() => {
      if (!ready) setError('Link expirado ou inválido. Volte e solicite um novo link de recuperação.');
    }, 10000);
    ...
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-5 bg-background">
        <div className="w-full max-w-sm rounded-2xl bg-card border border-border p-8 text-center space-y-4">
          ...
          <button onClick={() => navigate('/forgot-password')} className="w-full rounded-xl bg-primary text-primary-foreground font-medium py-3 text-sm transition-all active:scale-[0.98]">
            Solicitar novo link
          </button>
        </div>
      </div>
