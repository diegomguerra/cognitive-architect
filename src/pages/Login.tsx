    ...
    <div className="min-h-dvh flex flex-col items-center justify-center px-5 bg-background">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="font-mono text-xs text-muted-foreground">Performance cognitiva</p>
        </div>
        <span className="ml-3 border border-border rounded px-2 py-0.5 text-[10px] uppercase tracking-[0.3em] text-muted-foreground font-mono">
          secure
        </span>
      </div>

      <div className="w-full max-w-sm rounded-2xl bg-card border border-border p-6 sm:p-8">
        <h2 className="text-xl font-semibold text-foreground mb-1">{isSignUp ? 'Criar conta' : 'Entrar'}</h2>
        <p className="text-sm text-muted-foreground mb-6">{isSignUp ? 'Crie sua conta VYR' : 'Acesse sua conta'}</p>

        <div className="space-y-3 mb-5">
          <button
            onClick={() => handleOAuth('google')}
            className="w-full flex items-center justify-center gap-3 rounded-xl border border-border bg-background px-4 py-3 text-xs text-muted-foreground transition-transform active:scale-[0.98]"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
                minLength={6}
                className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/40 outline-none focus:ring-1 focus:ring-ring transition-colors pr-10"
              />
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-xl bg-primary text-primary-foreground font-medium py-3.5 text-sm transition-all active:scale-[0.98] hover:opacity-90 mt-2 disabled:opacity-50"
          >
      ...
      <p className="font-mono text-[10px] text-muted-foreground/50 mt-8 text-center">
